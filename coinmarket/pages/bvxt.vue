    ...
    <Content>
      <div class="item__container">
        <div class="item__left">
          <div
            class="item__lefttop item__projectdetail"
            v-if="$route.query.category === 'icoitem'"
          >
            <h1 class="item__projectdetail--h1">
              {{ removeAfterBracket($route.query.project_name) }}
            </h1>
            <p class="item__projectdetail--p">
              Introducing The BVX Liquidity Token: The Future of DeFi Liquidity and Prediction Markets
            </p>

            <p class="item__projectdetail--p">
              The BVX Liquidity Token is a revolutionary new cryptocurrency designed to enhance liquidity in
              the decentralized finance (DeFi) ecosystem while powering next-generation
              prediction markets. As a young and ambitious project, we have just launched
              our Initial Coin Offering (ICO), offering early adopters the opportunity to
              be part of something truly transformative.
            </p>

            <p class="item__projectdetail--p">
              Built for scalability and innovation, The BVX Liquidity Token leverages cutting-edge blockchain
              technology to facilitate seamless transactions, improve market efficiency,
              and create new opportunities in DeFi. Our vision extends beyond the
              present—future collaborations with major exchanges like Binance <img src="@/assets/imgs/binancelogo.png"/>, Bybit <img src="@/assets/imgs/bybit-logo-png_seeklogo-412982-removebg-preview.png"/>, and
              Uniswap <img src="@/assets/imgs/uniswap_unicorn.png"/> are on the horizon as we continue to grow.
            </p>

            <p class="item__projectdetail--p">
              At The BVX Liquidity Token, we aim high. Our goal is to establish a dominant position in the
              market, targeting a profit margin of at least $1 billion as our ecosystem
              expands. With strategic partnerships and relentless development, The BVX Liquidity Token is set
              to redefine the landscape of decentralized liquidity and predictive
              analytics.
            </p>

            <p class="item__projectdetail--p">
              Now is the time to get involved. Whether you're a DeFi enthusiast, trader,
              or investor, The BVX Liquidity Token offers a unique opportunity to engage with a dynamic and
              forward-thinking cryptocurrency. Join us on our journey as we shape the
              future of blockchain-powered finance.
            </p>

            <p class="item__projectdetail--p">
              Be part of the movement. Invest in The BVX Liquidity Token today and help drive the next
              evolution in DeFi liquidity and prediction markets!
            </p>
            ...
            <div class="item__labelmatch">
              <h2>Categories:</h2>
              <p>Platform, Internet-technology, Network, AI, Liquidity</p>
            </div>
          </div>
          ...
          <div class="item__leftbottom">
            <Table>
              ...
              >
                ...
                <div class="table__header--area" :style="{ 'grid-column': 'span 2' }">
                  {{ headerLabels[3] }}
                </div>
              ...
              <div
                class="table__header table__item"
                :style="{ gridTemplateColumns: gridTemplateColumns }"
              ...
              >
                ...
                <div
                  v-if="tradeData"
                  class="table__header--area"
                  :class="{
                    red: tradeData.type === 'Sell',
                    green: tradeData.type === 'Buy',
                  }"
                >
                  {{ tradeData.type }}
                </div>
                ...
                >
                  {{ formatCurrencyValue(tradeData.quantity) }}
                </div>
                <div
                  v-if="tradeData"
                  class="table__header--area"
        ...
        <div class="item__right">
          <div
            class="item__toprightboxsubject"
            v-if="this.$route.query.category === 'markets'"
          >
            <div class="item__toprightboxsubject--name" v-if="item_data.data">
              {{ item_data.data.displayName }}
            </div>
            <div class="item__toprightboxsubject--sub">
              <div
                class="item__toprightboxsubject--subleft item__toprightboxsubject--area"
              >
                <figure>
                  <img src="@/assets/imgs/binancelogo.png" />
                </figure>
                <div class="item__toprightboxsubject--subinterestingdata">
                  {{ item_data.data.blockchain.blockchain }}
                </div>
              </div>
              ...
              <div
                class="item__toprightboxsubject--subright item__toprightboxsubject--area"
              >
                <figure>
                  <img src="@/assets/imgs/uniswap_unicorn.png" />
                </figure>
                <div class="item__toprightboxsubject--subinterestingdata">
                  {{ item_data.data.quoteCurrencyName }}
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="item__toprightcontent">
            <div
              class="item__toprightgrid first top-spacing"
              v-if="$route.query.category === 'markets' && board_base.data"
            >
            ...
            <div
              class="item__toprightgrid second top-spacing"
              v-if="$route.query.category === 'markets'"
            >
              <Box
                :label="'max leverage'"
                :match="item_data.data.crossMargin.maxLeverage"
              />
              <Box
                :label="'price scale'"
                :match="item_data.data.symbolTradeLimit.priceScale"
              />
              <Box
                :label="'highest bid'"
                :match="item_data.data.symbolTradeLimit.highestBid"
              />
              <Box
                :label="'lowest ask'"
                :match="item_data.data.symbolTradeLimit.lowestAsk"
              />
            </div>
            <div
              class="item__toprightgrid third top-spacing"
              v-if="$route.query.category === 'markets'"
            >
              ...
              <Box
                :label="'fee'"
                :match="formatCurrencyValue(item_data.data.blockchain.withdrawFee)"
              />
            ...
            <div class="item__toprighttable top-spacing">
              <div class="item__toprighttable--body">
                <div class="item__toprighttable--tabarea">
                  <div
                    class="item__toprighttable--tab light-white"
                    @click="boardDummyDataGen('5M')"
                    :class="{ currentBoardView: currentBoardView === '5M' }"
                  >
                    5M
                  </div>
                  <div
                    class="item__toprighttable--tab light-white"
                    @click="boardDummyDataGen('1H')"
                    :class="{ currentBoardView: currentBoardView === '1H' }"
                  >
                    1H
                  </div>
                  <div
                    class="item__toprighttable--tab light-white"
                    @click="boardDummyDataGen('6H')"
                    :class="{ currentBoardView: currentBoardView === '6H' }"
                  >
                    6H
                  </div>
                  <div
                    class="item__toprighttable--tab light-white"
                    @click="boardDummyDataGen('24H')"
                    :class="{ currentBoardView: currentBoardView === '24H' }"
                  >
                    24H
                  </div>
                </div>
...
export default {
  ...
  data() {
    return {
      tableLabels: ["Date", "Type", "Type", "ratio", "USD", "volume", "shrooms", "txn"],
